...
- 비회원 사용자도 앱을 사용할 수 있게 해요.
- 비회원 사용 데이터는 서버 DB 대신 기기 저장소에 저장해요.
- 회원 사용자도 동일 데이터를 기기 저장소에 캐시로 함께 저장해요.
- 웹은 인증 완료 상태 또는 게스트 상태를 브리지로 전달받아 진입해요.
...
## 5. RN 책임
- `Splash -> Welcome -> Auth or Guest -> PermissionGate -> WebViewContainer` 흐름을 관리해요.
- 네이티브 헤더의 더보기 모달에서 계정 연동, 약관, 개인정보 처리 안내, 기기 저장 데이터 비우기, 앱 버전을 제공해요.
...
## 6. Web 책임
- 브리지로 전달받은 인증 상태 또는 게스트 상태를 기준으로 세션을 부트스트랩해요.
- 상태 확인 질문, 결과, 감정 일기, 기록 조회 UI를 렌더링해요.
- 서버와의 데이터 통신은 TanStack Query 기반으로 관리해요.
- 비회원 데이터는 브리지를 통해 앱 기기 저장소에 저장해요.
- 비회원이 만든 데이터는 동기화 대기 큐에도 같이 저장해요.
- 회원 데이터는 API 저장과 동시에 브리지를 통해 앱 기기 저장소에도 캐시해요.
- 계정 연동 시 앱이 동기화 대기 큐를 읽어 서버 DB와 맞춰요.
...
- 비회원 엔트리는 기기 저장소에 저장해요.
- 회원 엔트리는 서버 저장과 기기 캐시 저장을 동시에 처리해요.
